 ...
 * 655 输出二叉树
 */
public class algo_0655 {
    public List<List<String>> printTree(TreeNode root) {
        int high = getHigh(root);
        int num = (int)Math.pow(2, high) - 1;
        List<List<String>> list = new ArrayList<>(high);
        if(root == null)
            return list;
        ...
        dfs(root, 0, 0, list.get(0).size() - 1, list);
        return list;
    }

    private void dfs(TreeNode root, int depth, int start, int end, List<List<String>> list){
        if(root == null)
            return;
        if(start > end)
            return;
        ...
        dfs(root.left, depth + 1, start, mid - 1, list);
        dfs(root.right, depth + 1, mid + 1, end, list);
    }
